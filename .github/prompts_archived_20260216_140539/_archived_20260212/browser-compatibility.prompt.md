...
**Your Task**: Define browser support matrix, identify compatibility issues using caniuse.com, implement fixes (polyfills, feature detection, progressive enhancement), provide testing recommendations.

**Support Tiers** (tailor to project analytics):

- **Tier 1** (Full experience): Chrome, Firefox, Safari, Edge (last 2 versions)
- **Tier 2** (Core features): Older browsers (IE11, older mobile), some degradation acceptable
- **Tier 3** (Basic content): Text-only browsers, very old browsers

**Strategies**:

- **Feature Detection**: `if ('IntersectionObserver' in window)` not user-agent sniffing
- **Progressive Enhancement**: Core HTML/CSS works everywhere, JavaScript enhancements for capable browsers
- **Polyfills**: Load conditionally - core-js, IntersectionObserver, fetch polyfill
- **Transpilation**: Babel for ES6+ → ES5, configure targets in browserslist
- **CSS**: Autoprefixer for vendor prefixes, @supports for feature queries, fallback values
- **Graceful Degradation**: Advanced features degrade to basic working functionality

**Tools & Testing**:

- **caniuse.com**: Check feature support across browsers
- **browserslist**: Define targets in package.json
- **BrowserStack/LambdaTest**: Cloud browser testing
- **Lighthouse**: Desktop/mobile testing
...
**Success Criteria**: All Tier 1 browsers fully functional, Tier 2 core features work, polyfills loaded conditionally, no JavaScript errors in targets.
