...
Diagnose and eliminate unnecessary re-renders in ${selection} or ${file} to improve performance.

**Your Task**: Use React DevTools Profiler to identify excessive re-renders, diagnose root cause, apply targeted optimizations (React.memo/useMemo/useCallback), validate improvement.

**CRITICAL RULE**: Profile before optimizing! Don't apply memo/useMemo/useCallback without evidence of performance issue. Premature optimization adds complexity without benefit.

**Common Causes & Fixes**:

1. **Inline Objects/Arrays in JSX**: `style={{margin: 0}}` recreated each render → Move outside component or wrap in `useMemo`
2. **Inline Functions as Props**: `onClick={() => doThing()}` → Use `useCallback` only if child is wrapped in `React.memo`
3. **Context Value Changing**: `<Context value={{state, setState}}>` → Wrap value in `useMemo` to prevent all consumers re-rendering
4. **Parent Re-renders Expensive Child**: Parent state changes re-render all children → Wrap expensive child in `React.memo` to skip if props unchanged
5. **Unstable List Keys**: Using index as key → Use stable unique IDs from data
6. **Prop Reference Identity**: Passing new object/array reference each render → `useMemo` or move outside component

**Profiling Workflow**: Open React DevTools Profiler → Click record → Interact with slow UI → Stop recording → Review flamegraph for components rendering unnecessarily → Click component to see why it rendered.

**Tool Usage**: If `optimize_renders` MCP tool available, use it. Otherwise guide manual profiling.

**Standards**: [Vercel React Best Practices - Re-renders](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices).

**Success Criteria**: Profiler shows reduced render count, UI feels more responsive, optimizations justified by measurements.
