...
**Your Task**: Analyze component behavior (what users can do, what they see), detect test framework (Vitest/Jest), generate tests using React Testing Library patterns (Arrange-Act-Assert), cover edge cases (loading, errors, empty states).

**Test Structure (AAA Pattern)**:

- **Arrange**: Render component with props/context
- **Act**: Simulate user interactions with `userEvent` (click, type, select)
- **Assert**: Verify expected outcome (UI changes, callbacks called)

**Testing Library Query Priority** (accessibility-first):

1. `getByRole` - Prefer always (button, heading, textbox, link) - Tests accessibility
2. `getByLabelText` - Form inputs associated with labels
3. `getByPlaceholderText` - When no label exists
4. `getByText` - Non-interactive text content
5. `getByTestId` - Last resort only when above fail

**Framework Detection**: Inspect package.json for Vitest (`vi.fn()`, `describe`, `it`) or Jest (`jest.fn()`, `describe`, `it`). Import from `@testing-library/react` (render, screen), `@testing-library/user-event` (userEvent.click, type).

**Test Coverage Areas**:

- **Rendering**: Component renders with required props, displays correct content
- **Interactions**: Clicks, typing, form submission trigger expected behavior
- **Props**: Different prop values produce different outputs
- **Async**: Loading states, data fetching with `waitFor`, error handling
- **Edge Cases**: Empty states, null data, validation errors
- **Accessibility**: Keyboard navigation works, screen reader accessible (roles, labels)
...
it("submits form when user clicks submit", async () => {
  ...
  render(<LoginForm on Submit={onSubmit} />);
...
**Success Criteria**: Generated tests are copy-pasteable, use proper queries, cover user behavior, pass when component works correctly. 5. What error states exist? 6. Is this component accessible?
