...
Review ${selection} or ${file} (React functional components) for best practices, performance, accessibility, and maintainability.

**Your Task**: Analyze component structure, hooks usage, state management, identify anti-patterns, prioritize issues (Critical→Low), provide refactored code examples.

**Review Dimensions** (8 categories):

1. **Component Structure**: Single responsibility, proper organization (imports → props interface → state → computed → effects → handlers → render), clean separation of concerns
2. **State Management**: Appropriate useState/useReducer usage, state colocation (keep state close to usage), avoid prop drilling (use composition or Context)
3. **Performance**: Identify unnecessary re-renders, recommend React.memo/useMemo/useCallback only when profiled, check for expensive computations in render, suggest code splitting with React.lazy
4. **Accessibility**: Semantic HTML (`<button>` not `<div onClick>`), keyboard navigation, ARIA attributes, color contrast, screen reader support
5. **Error Handling**: Error boundaries, loading states, graceful degradation, null checks
6. **Testability**: Testable structure, avoid implementation details, proper separation for React Testing Library patterns
7. **TypeScript**: Proper prop types, avoid `any`, use inference, generic components when appropriate
8. **Anti-Patterns**: Mutating state, missing dependencies in useEffect, large components (split), inline styles overuse, prop drilling

**Tool Usage**: Use `review_component` MCP tool if available. Otherwise manual review.

**Standards**: [Vercel React Best Practices](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices) covering 40+ rules: waterfalls (avoid sequential fetching), bundle size (code splitting), re-renders (memo strategically), state (useState/Context/external), accessibility (semantic HTML).

**Output Format**: List issues grouped by category and severity. For each: specific problem, why it matters, refactored code example, priority level.

**Success Criteria**: Code follows Vercel patterns, no render performance issues, fully accessible, easily testable. 3. **Performance** - Memoization, code splitting, re-render optimization 4. **Accessibility** - WCAG compliance, semantic HTML, ARIA 5. **Error Handling** - Error boundaries, loading states, edge cases 6. **Testing** - Testability, React Testing Library patterns 7. **TypeScript** - Type safety, prop types, generics (if applicable) 8. **Anti-Patterns** - Common mistakes and code smells
