...
Migrate class component ${selection} or ${file} to functional component with hooks.

**Your Task**: Analyze class component (state, lifecycle, refs), incrementally convert to hooks, ensure tests pass, extract custom hooks for reusable logic.

**Migration Mapping**:

- `this.state = {}` → `const [state, setState] = useState({})`
- `this.setState({x: val})` → `setState(prev => ({...prev, x: val}))`
- `componentDidMount` → `useEffect(() => { /* mount logic */ }, [])`
- `componentDidUpdate` → `useEffect(() => { /* update logic */ }, [deps])`
- `componentWillUnmount` → `useEffect(() => { return () => { /* cleanup */ } }, [])`
- `this.refs.myRef` or `React.createRef()` → `const myRef = useRef()`
- Accessing context → `const value = useContext(MyContext)`
- Instance methods → Regular functions inside component or custom hooks
- `getDerivedStateFromProps` → `useMemo` or handle in render
- `shouldComponentUpdate` → `React.memo` wrapper

**Incremental Strategy**: Convert one piece at a time (state first, then lifecycle, then refs), run tests after each change, commit working code frequently.

**Custom Hook Extraction**: If patterns emerge (data fetching, subscriptions, form handling), extract to custom hooks for reusability.

**Testing**: Ensure all existing tests pass. Behavior should be identical before and after migration.

**Standards**: [Vercel React Best Practices](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices).

**Success Criteria**: Functional component with hooks, tests passing, no behavioral changes, code more concise.
