...
Perform security analysis of ${selection} or ${file} covering OWASP Top 10 and frontend security best practices.

**Your Task**: Scan for common vulnerabilities (XSS, injection, insecure auth, data exposure, vulnerable dependencies), prioritize by severity (Critical→Low), provide secure code fixes.

**Security Dimensions**:

1. **XSS Prevention**: Never use `dangerouslySetInnerHTML` without sanitization (DOMPurify). Escape user input. Implement Content Security Policy headers.
2. **Authentication**: Store tokens in httpOnly cookies (NOT localStorage - vulnerable to XSS). Implement proper session management, token refresh, logout.
3. **Authorization**: Verify permissions on every request. Never rely on client-side checks alone. Hide UI but always validate server-side.
4. **Dependency Vulnerabilities**: Run `npm audit`, fix Critical/High vulnerabilities immediately. Use `npm audit fix` or update packages manually.
5. **Data Protection**: Never expose API keys, secrets, or credentials in client code. Use environment variables, server-side proxies. Encrypt sensitive data in transit (HTTPS only).
6. **Injection**: Parametrize SQL queries server-side. Validate and sanitize all user inputs. Use prepared statements.
7. **CORS Configuration**: Restrict allowed origins, don't use wildcard `*` in production.
8. **Sensitive Data Exposure**: Mask passwords, credit cards in logs. Don’t log auth tokens. Use secure headers (X-Content-Type-Options, X-Frame-Options).

**Critical Issues** (fix immediately):

- XSS via unescaped user content
- API keys/secrets in GitHub repo or client bundle
- Tokens stored in localStorage
- Critical npm audit vulnerabilities
- Missing authentication on sensitive routes

**Tools**: `npm audit`, ESLint security plugins (eslint-plugin-security), OWASP ZAP for penetration testing.

**OWASP Top 10 Reference**: Injection, Broken Auth, Sensitive Data Exposure, XXE, Broken Access Control, Security Misconfiguration, XSS, Insecure Deserialization, Using Components with Known Vulnerabilities, Insufficient Logging.

**Success Criteria**: All Critical/High vulnerabilities fixed, `npm audit` shows no critical issues, authentication secured with httpOnly cookies, no secrets in code.
