...
Establish performance budgets and configure CI/CD enforcement to prevent regressions.

**Your Task**: Measure baseline performance, set realistic budget limits, configure enforcement in build process and CI pipeline.

**Budget Targets** (adjust based on baseline):

- **Bundle Size**: Total JS ≤250KB gzipped CSS ≤50KB gzipped, images ≤500KB per page
- **Load Metrics**: FCP ≤1.8s, LCP ≤2.5s, TTI ≤3.8s, Speed Index ≤3.4s
- **Core Web Vitals**: LCP ≤2.5s, INP ≤200ms, CLS ≤0.1
- **Lighthouse Scores**: Performance ≥90, Accessibility ≥95, Best Practices ≥90, SEO ≥90

**Implementation**:

**Lighthouse CI**: Create `.lighthouserc.json` with assertions:
...
**Build Tool Limits**: Detect and configure based on stack:

- Vite: `rollupOptions.output.assetFileNames` + manual checks
- Webpack: `performance: { maxAssetSize: 250000, maxEntrypointSize: 250000 }`
- Next.js: Use `@next/bundle-analyzer`

**CI Integration**: Add to GitHub Actions, GitLab CI, or other pipeline. Fail builds exceeding budgets. Example: `npm run lighthouse-ci -- --upload.target=temporary-public-storage`.

**Monitoring**: Track historical trends, alert team on violations, update budgets quarterly as performance improves.

**Standards**: [Web Quality Skills - Performance Budgets](https://github.com/addyosmani/web-quality-skills).

**Success Criteria**: Budgets enforced in CI, builds fail on violations, team has clear performance targets.
