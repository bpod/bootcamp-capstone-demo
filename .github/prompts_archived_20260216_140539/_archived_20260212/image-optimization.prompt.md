...
Optimize images in ${selection}, ${file}, or entire project to reduce LCP and page weight by 50-80%.

**Your Task**: Analyze current images, convert to modern formats (WebP/AVIF), implement responsive sizing with srcset, add lazy loading, set explicit dimensions to prevent CLS.

**Impact**: Images are typically 50% of page weight and 77% of LCP elements. Proper optimization reduces LCP by 30-50% and bandwidth by 60-80%.

**Optimization Strategy**:

**Format Conversion**: JPEG/PNG → WebP (60-80% smaller at equivalent quality). PNG with transparency → WebP. GIF animations → WebP animated or MP4 video. Use `<picture>` element for fallback support. Recommend 85-90% quality for WebP.

**Responsive Images**: Implement `srcset` with multiple sizes (e.g., 480w, 768w, 1200w, 1600w) and `sizes` attribute to serve appropriately sized images per viewport. Generate 2-3 breakpoints minimum. Use `<picture>` for art direction (different crops per size).

**Lazy Loading**: Add `loading="lazy"` to all below-the-fold images. CRITICAL: Never lazy-load LCP image - use `loading="eager"` + `fetchpriority="high"` and preload it: `<link rel="preload" as="image" href="hero.webp">`.

**Dimensions**: Set explicit `width` and `height` attributes on all images to reserve space and prevent CLS. Use `aspect-ratio` CSS for responsive scaling.
...
**Tool Detection**: Adapt recommendations to user's stack - Vite (`vite-imagetools`), Webpack (`image-webpack-loader`), Next.js (`next/image` component), Astro (built-in `<Image>`).

**Standards**: [Web Quality Skills - Image Performance](https://github.com/addyosmani/web-quality-skills).

**Success Criteria**: LCP improved ≥500ms, image payload reduced ≥30%, zero CLS from images.
