...
Perform comprehensive accessibility audit of ${selection}, ${file}, or ${input:url} following WCAG 2.1 Level AA standards.

**Your Task**: Review code or URL for accessibility violations, categorize by severity (Critical/High/Medium/Low), provide before/after code fixes, suggest keyboard and screen reader testing.

**Audit Coverage**:

- **Semantic HTML** (WCAG 4.1.2): Use `<button>`, `<nav>`, `<main>`, `<article>`, headings hierarchy, not generic `<div>` for interactive elements
- **Keyboard Navigation** (WCAG 2.1.1): All interactive elements accessible via Tab, visible focus indicators (3:1 contrast), no keyboard traps, logical tab order
- **ARIA** (WCAG 4.1.2, 4.1.3): Proper roles, `aria-label` on icon-only buttons, `aria-labelledby` with valid IDs, `aria-invalid` on errors, `aria-live` for dynamic content
- **Color Contrast** (WCAG 1.4.3): 4.5:1 for normal text, 3:1 for large text (18pt+ or 14pt bold), 3:1 for UI components
- **Forms** (WCAG 3.3.2): Each input has associated `<label>` or `aria-label`, error messages linked with `aria-describedby`, validation feedback accessible
- **Images** (WCAG 1.1.1): Alt text on all content images, `alt=""` for decorative images, avoid "image of" or "picture of"
- **Focus Management** (WCAG 2.4.3): Modals trap focus, popups return focus on close, skip links provided

**Tool Usage**: Use accessibility audit MCP tool if available. Otherwise recommend: `npx pa11y ${input:url} --standard WCAG2AA` (for URLs) or `eslint-plugin-jsx-a11y` (for React).

**Critical Issues** (fix immediately): Keyboard navigation failure, missing alt text, forms without labels, contrast ratio <3:1.

**Output Format**: List issues grouped by severity. For each: WCAG criterion, code location, specific fix with before/after example, screen reader impact.

**Standards**: [Web Quality Skills - Accessibility](https://github.com/addyosmani/web-quality-skills).

**Success Criteria**: All Critical/High issues resolved, code passes automated WCAG 2.1 AA checks, keyboard-only navigation functional.
