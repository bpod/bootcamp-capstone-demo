...
Refactor state management in ${selection} or ${file} using appropriate pattern based on scope and complexity.

**Your Task**: Analyze current state placement, identify issues (prop drilling, state too high/low, complex updates), choose simplest effective pattern, refactor incrementally.

**Decision Tree** (use simplest that works):

1. **Single component**: `useState` locally where used
2. **Few nearby components**: Lift state to common parent, pass props down
3. **Prop drilling 3+ levels**: Context API for shared data
4. **Complex interdependent state**: `useReducer` for state machines or related updates
5. **Global app-wide state**: External library (Zustand, Jotai, Redux Toolkit)

**Patterns**:

- **State Colocation**: Keep state as local as possible. Only lift when multiple components need it.
- **Lifting State**: Move state to lowest common ancestor that needs it.
- **Context**: For data used by many deeply nested components (theme, auth, i18n). Don't overuse - causes all consumers to re-render.
- **useReducer**: For complex state with multiple interdependent values or complex update logic.
- **Composition**: Pass components as children/props to avoid lifting state unnecessarily.
- **External State**: For true global state (user profile, app settings). Prefer Zustand (simple) or Jotai (atomic) over Redux for new projects.

**Anti-Patterns**: Global state for everything, props drilling >3 levels, useState for complex interdependent state, multiple Contexts for same feature.

**Standards**: [Vercel React Best Practices - State Management](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices).

**Success Criteria**: State appropriately scoped, no unnecessary prop drilling, simpler state updates, tests passing.
