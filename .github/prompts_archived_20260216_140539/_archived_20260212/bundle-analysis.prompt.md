...
Analyze JavaScript bundle size and provide optimization strategies to reduce payload and improve TTI.

**Your Task**: Examine build output, identify large dependencies, detect code splitting opportunities, recommend alternatives to heavy libraries.

**Impact**: Every 100KB of JavaScript adds ~1s to TTI on mid-tier mobile. Target: TTI ≤3.8s, main bundle ≤170KB gzipped.

**Analysis Process**: Detect build tool from package.json, run appropriate bundle visualizer:

- Vite: `npx vite-bundle-visualizer`
- Webpack: `npx webpack-bundle-analyzer dist/stats.json`
- Next.js: Enable bundle analyzer in next.config.js

**Optimization Strategies**:

**Code Splitting**: Implement route-based splitting with `React.lazy()` and `Suspense`. Split vendor bundles from app code. Dynamic import for heavy libraries used conditionally (e.g., charts, rich text editors). Example: `const Chart = lazy(() => import('./Chart'));`.

**Tree Shaking**: Import specific functions not entire libraries: `import { debounce } from 'lodash-es'` not `import _ from 'lodash'`. Verify `sideEffects: false` in package.json. Avoid barrel exports (index.js re-exporting everything).

**Dependency Audit**: Use `npx depcheck` to find unused dependencies. Check bundlephobia.com before adding new packages. Replace heavy alternatives: moment.js (231KB) → date-fns (78KB), lodash(full) → individual functions.

**Framework-Specific**: Provide config for detected tool:

- Vite: `rollupOptions.output.manualChunks`
- Webpack: `optimization.split Chunks.cacheGroups`
- Next.js: Automatic code splitting (verify dynamic imports)

**Standards**: [Vercel React Best Practices - Bundle Size](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices).

**Success Criteria**: Initial JS payload ≤170KB gzipped, largest chunk ≤100KB, identify ≥3 optimizations with size savings.
