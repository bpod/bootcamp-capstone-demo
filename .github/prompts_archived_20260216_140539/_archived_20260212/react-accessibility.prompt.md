...
Implement accessible React components in ${selection} or ${file} following WCAG 2.1 Level AA standards.

**Your Task**: Audit JSX for accessibility violations, provide React-specific patterns (semantic JSX, hooks for focus management), integrate jest-axe testing, ensure keyboard navigation.

**React Accessibility Patterns**:

- **Semantic JSX**: Use `<button>` not `<div onClick>`, `<nav>` for navigation, proper heading hierarchy
- **Focus Management**: `useRef` + `.focus()` after dynamic updates, manage focus in modals/popups (react-focus-lock), return focus on close
- **Keyboard Handlers**: `onKeyDown` checking Enter/Space for custom interactions, ensure all interactive elements keyboard accessible
- **Forms**: `useId()` hook for label/input association, `aria-describedby` for errors, `aria-invalid` on validation failures
- **Live Regions**: `aria-live="polite"` for status messages, `aria-live="assertive"` for errors/alerts
- **Modals**: `role="dialog"`, `aria-modal="true"`, `aria-labelledby` for title, focus trap, Esc to close

**Testing Integration**: Add jest-axe to component tests:
...
**Common JSX Issues**: Missing alt on images, buttons without accessible text, inputs without labels, insufficient color contrast, keyboard traps, incorrect ARIA usage.

**Standards**: [Web Quality Skills - Accessibility](https://github.com/addyosmani/web-quality-skills) and [Vercel React Accessibility](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices).

**Success Criteria**: Jest-axe tests pass, keyboard navigation functional, screen reader testing recommended.
