...
Guide systematic debugging of frontend issue following: Reproduce → Isolate → Hypothesize → Test → Fix → Verify.

**Your Task**: Help user debug issue by gathering context (expected vs actual behavior, browser, reproducibility), isolating root cause using DevTools, providing targeted fix, verifying solution.

**Debugging Workflow**:

1. **Reproduce**: Confirm issue happens consistently. Note exact steps, browser/device, any error messages.
2. **Gather Info**: Check browser console for errors, Network tab for failed requests, React DevTools for component state.
3. **Isolate**: Narrow down to specific component/function/line. Use `debugger;` statement or DevTools breakpoints. Check recent code changes.
4. **Hypothesize**: Form testable theory about cause (state mutation? missing dependency? race condition?).
5. **Test**: Add logging, use debugger to inspect variables, try minimal reproduction.
6. **Fix**: Implement targeted solution based on root cause.
7. **Verify**: Confirm fix works, check for side effects or regressions.

**Chrome DevTools Quick Reference**:

- **Console**: `Cmd+Option+J` (Mac) / `Ctrl+Shift+J` (Win) - View errors, run JavaScript
- **Sources**: Set breakpoints, step through code, inspect variables
- **Network**: Check failed requests, response codes, timing
- **React DevTools**: Inspect component props/state, trace re-renders with Profiler
- **Performance**: Record and analyze runtime performance

**Common Issue Patterns**:

- **State not updating**: Mutating state instead of immutable update (`state.push()` → `setState([...state, item])`)
- **Infinite loop/re-renders**: Missing or incorrect useEffect dependencies, inline object/function creation
- **API call failing**: Check Network tab for CORS, 404, 500 errors. Verify endpoint URL, auth headers.
- **Component not rendering**: Check conditional rendering logic, verify props passed correctly
- **Event handler not firing**: Check event binding, event propagation, disabled state

**Critical Rule**: Use data to guide debugging. Don't guess or make random changes. Add logging, use debugger, form hypotheses based on evidence.

**Success Criteria**: Issue root cause identified, fix implemented and verified, underlying problem explained to user.
