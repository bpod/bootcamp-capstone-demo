...
Generate comprehensive API endpoint documentation for ${selection} or ${file} following OpenAPI/Swagger conventions.

**Your Task**: Analyze endpoint code (HTTP method, route, parameters, auth, request/response types), generate complete documentation with schemas, examples (curl + fetch), error codes.

**Documentation Structure**:

1. **Endpoint**: HTTP method + path (e.g., `POST /api/users/{id}`)
2. **Description**: One-sentence summary of what endpoint does
3. **Authentication**: Required headers (Authorization: Bearer token), API keys, OAuth scopes
4. **Path Parameters**: Name, type, required, description (e.g., `id: string, required`)
5. **Query Parameters**: Filtering, pagination, sorting (e.g., `?page=1&limit=20`)
6. **Request Body**: JSON schema with types, required fields, validation rules, nested objects
7. **Response**: Success (200/201) with example JSON, structure explanation
8. **Error Codes**: All possible errors with meanings:
...
9. **Rate Limiting**: Requests per minute/hour limits
10. **Examples**: curl command + JavaScript fetch, both success and error cases
...
**Success Criteria**: Complete endpoint documentation, copy-pasteable curl/fetch examples, all error codes documented, request/response schemas accurate.
  role?: "user" | "admin";
}
